...
func TestTokenizeMatchesComfyUI(t *testing.T) {
	...
	for _, name := range []string{"short", "portrait", "weighted"} {
		...
		for i := range got.IDs {
			if got.IDs[i] != want.IDs[i] || got.Weights[i] != want.Weights[i] {
				t.Fatalf("%s: token %d is %d×%v, want %d×%v", name, i, got.IDs[i], got.Weights[i], want.IDs[i], want.Weights[i])
			}
		}
		if got.Start != 34 {
...
func TestTokenWeights(t *testing.T) {
	got := tokenWeights(escapeImportant(`a (red) ((big)) (cat:1.3) \(x\)`), 1)
	want := []segment{{"a ", 1}, {"red", 1.1}, {" ", 1}, {"big", float32(1.1 * 1.1)}, {" ", 1}, {"cat", 1.3}, {" \x00\x02x\x00\x01", 1}}
	if len(got) != len(want) {
		t.Fatalf("%v", got)
	}
	for i := range got {
