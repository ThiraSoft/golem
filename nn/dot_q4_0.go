...
// eight. rowTile then cuts the output, so that the slice of weights the columns
...
// matVecQ4_0Rows computes rows [start, end) on the caller's thread, four
// columns of the batch at a time.
//
// The order of the loops is the whole of it: a tile of rows on the outside, a
// stretch of inputs next, then the columns, then the rows of the tile. What
// that leaves in the cache is one stretch of four columns' activations — eight
// kilobytes — and the slice of weights the tile covers, and both are read from
// there by everything inside.
func matVecQ4_0Rows(w []byte, b *Batch, inputs int, ys [][]float32, start, end int) {
	...
	if inputs <= kTile {
		...
		for c := b.Size - b.Size%4; c < b.Size; c++ {
			for r := start; r < end; r++ {
