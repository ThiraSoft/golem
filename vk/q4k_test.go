...
func aQ4_K(tb testing.TB) (*tensors.GGUF, nn.Matrix) {
	...
	for _, env := range []string{"GOLEM_MODEL_Q4KM", "GOLEM_MODEL_QWEN_Q4KM", "GOLEM_MODEL_12B_Q4KM"} {
		...
		for _, name := range []string{"blk.0.ffn_gate.weight", "blk.0.ffn_up.weight", "blk.0.attn_q.weight"} {
			t, err := g.Get(name)
			if err != nil || t.DType != "Q4_K" {
				continue
			}
			return g, nn.Matrix{Data: t.Raw, Quant: nn.Q4_K, Cols: t.Shape[0], Rows: t.Shape[1]}
		}
		g.Close()
	}
	tb.Skip("set GOLEM_MODEL_Q4KM to a Q4_K_M checkpoint to run the Q4_K kernel tests")
...
// q4kAgainstQ8_0 is the product the tiled kernel actually computes: the weights
...
func q4kAgainstQ8_0(m nn.Matrix, batch *nn.Batch, columns int) [][]float32 {
	...
	stride := m.Cols / nn.SuperBlock * 144
	for i := 0; i < m.Rows; i++ {
		nn.DequantizeQ4_K(m.Data[i*stride:(i+1)*stride], m.Cols, row)
		for c := 0; c < columns; c++ {
			var sum float32
			for j := range row {
				sum += row[j] * x[c][j]
...
func TestVulkanQ4KTiledMatchesCPU(t *testing.T) {
	...
	for _, columns := range []int{64, 128, 256, 512} {
		t.Run(itoa(columns), func(t *testing.T) {
			batch := columnsOf(m.Cols, columns)
			want := q4kAgainstQ8_0(m, batch, columns)
