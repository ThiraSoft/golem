...
// aPTQ1_0 opens one PTQ1_0 matrix out of the Bonsai checkpoint.
func aPTQ1_0(tb testing.TB, tensorName string) (*tensors.GGUF, nn.Matrix) {
	tb.Helper()
	...
	if _, err := os.Stat(path); err != nil {
		tb.Skipf("PTQ1_0 model not found at %s", path)
	}
...
// TestVulkanPTQ10MatMulMatchesCPU is shaders/matmul_coop.comp under -DPTQ10 at every
// tiled width the door builds, on real tensors from the Bonsai checkpoint.
func TestVulkanPTQ10MatMulMatchesCPU(t *testing.T) {
	d := open(t)
	defer d.Close()
	if !d.Coopmat() {
		...
		t.Run(tensorName, func(t *testing.T) {
			...
			for _, columns := range []int{32, 64, 128, 256, 512} {
				t.Run(itoa(columns), func(t *testing.T) {
					...
					mm, err := NewMatMulQuant(d, m.Data, m.Rows, m.Cols, columns, true, nn.PTQ1_0)
					if err != nil {
						t.Fatal(err)
					}
					defer mm.Close()
