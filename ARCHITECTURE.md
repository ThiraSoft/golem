...
| `compress/` | the `.golem` format: a trellis that decodes two weights a state out of a small trained table, at 3.19 bits a weight (H3G) and 4.19 (H4G), the salience scale its columns are measured against, and the packing a shader reads. `compress/README.md` says what the format is and which of its rules a reader has to keep. |
...
Which kernel a pass runs is the width's business. A token and a short stretch draw the mat-vec, one row of the answer to a team of eight lanes; a stretch above eight draws a tiled product instead, which stages both operands in shared memory and keeps a tile of the answer in registers. `.golem`'s own kernels are laid out the same way and for a different reason: there the tile is not about the weight traffic — a trellis weight is under half a byte — but about the decode, which the mat-vec pays once per column of the pass and the tile pays once for the whole tile. Both read their operands sixteen bytes at a time, and both fetch a step of the walk into registers before the step they are computing, so that the card waits for memory with a step of arithmetic in hand rather than at a barrier.
